...
def get_yfull_df(data: Union[IO, PathLike]) -> pd.DataFrame:
    ...
    df[2] = (
        df[2].apply(lambda confidence: confidence_to_bool(confidence)).astype("bool")
    )
...
def add_yfull(
    ...
    file: Path = Argument(
        ..., exists=True, dir_okay=False, help="The YFull STR file for the kit."
    ),
) -> None:
    """Add a YFull kit to the Kits STR database."""
    ...
    yfull_series = pd.Series(new_vals).apply(
        lambda lst: [int(x) for x in lst] if isinstance(lst, list) else None
    )
...
def remove(
    ...
    """Remove kit(s) from the Kits STR database."""
    ...
    echo(f"Found {orig_num_kits:,} kits in Kits STR database.")
    ...
    echo(f"Will be {len(kits_df):,} kits in Kits STR database.")

    if not confirm("Proceed?"):
        secho("Not writing kits to database.", fg=colors.YELLOW)
        return

    store_db(kits_str_path, kits_df)
    echo(f"Kits STR database written to `{kits_str_path}`.")
...
def fetch_ftdna(
    ...
    page_size: int = Option(
        500, "--page-size", "-p", help="The page size to use when fetching kits."
    ),
) -> None:
    ...
    try:
        ...
        kits_df = ftdna_fetch_kits(
            url, page_size=page_size, http_timeout=15 + 0.05 * page_size
        )
...
def analyze(
    ...
    """Compare matches in the Kits STR database."""
    ...
    def get_abs_diffs(kit_s: pd.Series) -> pd.Series:
        def get_diff(a: List[int], b: List[int]) -> List[int]:
            ...
            min_diff = get_comb_diff(
                min(
                    (
                        [
                            (i, j, get_val_diff(a_val, b_val))
                            for (i, a_val), (j, b_val) in zip(a_comb, b_comb)
                        ]
    ...
    results_df = pd.concat(
        [
            ...
            rel_dist_cis.astype("float32").rename(
                f"Relative Distance (CI, {rel_dist_cl:%})", inplace=True
            ),
            rel_dist_mins.astype("float32").rename(
                "Relative Distance (min)", inplace=True
            ),
            rel_dist_maxs.astype("float32").rename(
                "Relative Distance (max)", inplace=True
            ),
        ],
        axis=1,
    )
