...
age_bounds_pattern = re.compile(
    r"formed CI (?P<age_cl>\d+)% (?P<age_min>\d+)<->(?P<age_max>\d+) ybp, TMRCA CI (?P<tmrca_cl>\d+)% (?P<tmrca_min>\d+)<->(?P<tmrca_max>\d+) ybp"
)


@app.command()
def download_yfull(
    haplogroup: str = Option(
        ..., "--haplogroup", "--hg", help="The haplogroup of the subtree to download."
    ),
) -> None:
    ...
    if not response.ok:
        secho(
            f"Haplogroup {haplogroup} not found in YFull tree.", fg=colors.RED, err=True
        )
        raise Exit(1)
    ...
    def traverse_tree(
        tree_ul: Tag, parent_haplogroup: Optional[str] = None
    ) -> Iterable[pd.Series]:
        ...
        for item_li in item_li_list:
            ...
            if age_span is not None:
                ...
                if age_match:
                    ...
                    secho(
                        f"WARNING: unexpected format of text for haplogroup age: '{age_text}'",
                        fg=colors.YELLOW,
                        err=True,
                    )
                ...
                if age_bounds_match:
                    ...
                    secho(
                        f"WARNING: unexpected format of text for haplogroup age bounds: '{age_bounds_text}'",
                        fg=colors.YELLOW,
                        err=True,
                    )
...
@app.command()
def prune_tree(
    haplogroups: str = Argument(
        ..., help="The reg-exp that matches haplogroups to be kept."
    ),
) -> None:
    ...
    echo(
        f"Pruned tree has {len(tree_df):,} haplogroups ({retained_fraction:.1%} retained)."
    )
...
@app.command()
def hg(
    use_snp_db: bool = Option(
        False, "--snp", help="Use the local SNP database to look up kits. (default)"
    ),
    use_str_db: bool = Option(
        False, "--str", help="Use the local STR database to look up kits."
    ),
    self_haplogroup: Optional[str] = Option(
        None, "--haplogroup", "--hg", help="The haplogroup to look up."
    ),
    self_kit: Optional[str] = Option(
        None, "--kit", "-k", help="The kit whose haplogroup to look up."
    ),
) -> None:
    ...
    if self_kit is not None:
        ...
        if kits_df is None:
            secho(
                f"ERROR: Kits {kits_db_kind} database does not exist.",
                fg=colors.RED,
                err=True,
            )
            raise Exit(1)
    ...
    self_kit_hg = (
        kits_df.loc[self_kit, "Haplogroup"] if self_kit is not None else self_haplogroup
    )
