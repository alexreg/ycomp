...
def get_yfull_df(
    data: Union[IO, PathLike]
) -> Tuple[Optional[str], List[str], pd.DataFrame]:
    df: pd.DataFrame = pd.read_csv(
        data, sep=";", index_col=0, header=None, names=[0, 1, 2, 3, 4]
    )
    ...
    fixed_cols_df = pd.DataFrame(
        (
            fix_cols(read_count, rating)
            for read_count, rating in zip(snps_df[2], snps_df[3])
        ),
        index=snps_df.index,
        columns=[2, 3],
    )
...
def add_yfull(
    ...
    group: Optional[str] = Option(
        None, "--group", help="The group within which the sample clusters."
    ),
    ancestor: Optional[str] = Option(
        None, "--ancestor", help="The earliest known patrilineal ancestor."
    ),
    country: Optional[str] = Option(
        None, "--country", help="The country from which the ancestor came."
    ),
    haplogroup: Optional[str] = Option(
        None, "--haplogroup", help="The haplogroup of the sample."
    ),
    file: Path = Argument(
        ..., exists=True, dir_okay=False, help="The YFull SNP file for the kit."
    ),
) -> None:
    ...
    if kit is None:
        ...
        if not match:
            raise BadParameter(
                "Could not infer kit name from filename; specify it explicitly.",
                param=kit,
            )
        kit = match.group(1)
...
@app.command()
def fetch_ftdna(
    ftdna_group: str = Option(
        ..., "--group", "-g", help="The name of the FTDNA group to fetch kits from."
    ),
    page_size: int = Option(
        250, "--page-size", "-p", help="The page size to use when fetching kits."
    ),
) -> None:
    ...
    try:
        ...
        kits_df = ftdna_fetch_kits(
            url, page_size=page_size, http_timeout=15 + 0.2 * page_size
        )
    ...
    if "Paternal Ancestor Name" in kits_df:
        kits_df["Paternal Ancestor Name"] = kits_df["Paternal Ancestor Name"].astype(
            "string"
        )
    else:
        kits_df["Paternal Ancestor Name"] = pd.Series(
            np.nan, index=kits_df.index, dtype="string"
        )
    ...
    def expand_row(row: Any) -> pd.Series:
        ...
        cols = (
            dict(
                filter(
                    None, (get_snp_value(call) for call in cast(str, row).split(", "))
                )
            )
            if pd.notna(row)
            else None
...
def merge_str_db() -> None:
    ...
    str_data_df = kits_str_df[
        ["Group", "Paternal Ancestor Name", "Country", "Haplogroup"]
    ]
...
def analyze(
    ...
    snp_max_age: int = Option(
        3500, "--max-age", help="The maximum age of SNPs to consider."
    ),
    haplogroup: Optional[str] = Option(
        None, "--haplogroup", help="The haplogroup clade, to filter by."
    ),
    haplogroup_max_diff: Optional[int] = Option(
        None,
        "--haplogroup-max-diff",
        help="The maximum difference between generations in the haplogroup tree, to filter by.",
    ),
    output_file: Path = Option(
        "ycomp-analysis-snp.csv",
        "--output",
        "-o",
        dir_okay=False,
        help="The (CSV) file to write the analysis to.",
    ),
) -> None:
    ...
    kits_df = kits_df[
        kits_df.apply(
            lambda kit: kit_matches_lineage(
                tree_df,
                self_kit_lineage,
                kit,
                haplogroup=haplogroup,
                haplogroup_max_diff=haplogroup_max_diff,
            ),
            axis=1,
        )
    ]
    ...
    def get_assumed_shared_snps(kit_s: pd.Series) -> pd.Series:
        return float_or(
            float_and(self_kit_s, float_isna(kit_s)), float_and(self_kit_isna_s, kit_s)
        )
    ...
    results_df = pd.concat(
        [
            ...
            num_comp_shared_snps.astype("int32").rename(
                "Shared SNPs (# compared)", inplace=True
            ),
            frac_shared_snps.astype("float32").rename(
                "Shared SNPs (fraction)", inplace=True
            ),
            list_assum_shared_snps.rename("Assumed Shared SNPs", inplace=True),
            num_assum_shared_snps.astype("int32").rename(
                "Assumed Shared SNPs (#)", inplace=True
            ),
            num_comp_assum_shared_snps.astype("int32").rename(
                "Assumed Shared SNPs (# compared)", inplace=True
            ),
            frac_assum_shared_snps.astype("float32").rename(
                "Assumed Shared SNPs (fraction)", inplace=True
            ),
            list_all_shared_snps.rename("All Shared SNPs", inplace=True),
            num_all_shared_snps.astype("int32").rename(
                "All Shared SNPs (#)", inplace=True
            ),
            num_comp_all_shared_snps.astype("int32").rename(
                "All Shared SNPs (# compared)", inplace=True
            ),
            frac_all_shared_snps.astype("float32").rename(
                "All Shared SNPs (fraction)", inplace=True
            ),
        ],
        axis=1,
    )
