...
def fetch_ftdna(
	...
	page_size: int = Option(250, "--page-size", "-p", "The page size to use when fetching kits."),
) -> None:
	...
	try:
		...
		kits_df = download_ftdna_kits(url, page_size = page_size, http_timeout = 15 + 0.2 * page_size)
	...
	kits_df = pd.concat(
		[
			...
			kits_df["Short Hand"].rename("Haplogroup"),
			expanded_rows,
		],
		axis = 1,
	)
