...
def get_common_lineage(a_lineage: Sequence[str], b_lineage: Sequence[str]) -> Sequence[str]:
	return list(hgs[0] for hgs in takewhile(lambda hgs: hgs[0] == hgs[1], zip(reversed(a_lineage), reversed(b_lineage))))


def kit_matches_lineage(tree_df: pd.DataFrame, a_lineage: Sequence[str], b: pd.Series, *, haplogroup: Optional[str], haplogroup_max_diff: Optional[int]) -> bool:
