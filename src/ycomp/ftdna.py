...
def ftdna_normalize_columns(kits_df: pd.DataFrame) -> None:
    if "Last Name" in kits_df.columns:
        if "Paternal Ancestor Name" not in kits_df.columns:
            kits_df.rename(
                columns={"Last Name": "Paternal Ancestor Name"}, inplace=True
            )
        else:
            kits_df["Paternal Ancestor Name"] = kits_df[
                "Paternal Ancestor Name"
            ].fillna(kits_df["Last Name"])
...
def ftdna_fetch_kits(
    url: str, *, page_size: Optional[int] = None, http_timeout: Optional[float] = None
) -> pd.DataFrame:
...
def ftdna_refresh(
    cookies: Optional[Cookies], *, http_timeout: float
) -> Optional[Cookies]:
...
def ftdna_signin(
    username: str, password: str, *, http_timeout: float
) -> Optional[Cookies]:
    ...
    async def run() -> Optional[Cookies]:
        ...
        userInfoForm: ElementHandle = await page.waitForSelector(
            "form[name = userInfoForm]"
        )
        kitNumberInput: ElementHandle = await userInfoForm.querySelector(
            "input[name = kitNumber]"
        )
        passwordInput: ElementHandle = await userInfoForm.querySelector(
            "input[name = password]"
        )
        submitButton: ElementHandle = await userInfoForm.querySelector(
            "button[type = submit]"
        )
        ...
        wait_for_navigation = asyncio.ensure_future(
            page.waitForNavigation(waitUntil="networkidle0")
        )
        wait_for_error_message = asyncio.ensure_future(
            page.waitForSelector("div#error-message")
        )
